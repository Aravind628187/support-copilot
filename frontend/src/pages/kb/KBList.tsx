...
export function KBListPage() {
  ...
  const { data, isPending, isError, refetch } = useQuery({
    ...
    queryFn: () => listArticles({ search: debouncedSearch || undefined }),
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (article) => {
      showToast({ variant: 'success', message: 'Article created' });
      reset();
      setCreateOpen(false);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="max-w-sm">
        <Input id="kb-search" placeholder="Search articles…" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>

      {isError ? (
        <ErrorState message="Could not load the knowledge base." onRetry={() => refetch()} />
      ) : isPending ? (
          ...
          {data.items.map((article) => (
            ...
            >
              <CardBody className="pt-4">
                <h3 className="mb-1 font-medium text-ink-950 dark:text-ink-100">{article.title}</h3>
                <p className="mb-2 line-clamp-2 text-sm text-ink-600 dark:text-ink-400">{article.content}</p>
      ...
      <Modal isOpen={createOpen} onClose={() => setCreateOpen(false)} title="New knowledge base article">
        <form onSubmit={handleSubmit((v) => createMutation.mutate(v))} className="flex flex-col gap-4" noValidate>
          ...
          <Input
            id="tags"
            label="Tags"
            hint="Comma-separated, e.g. billing, refund"
            onChange={(e) => {
